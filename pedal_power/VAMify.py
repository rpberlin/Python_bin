...
if __name__ == '__main__':
    ...
    time_minutes = np.array(range(0,len(sPath)))/60
    speed = np.diff(sPath)*3.6
    speed = np.append(speed[0], speed)
    smoothdx = 5
    smoothspeed = speed

    for i in range(smoothdx,len(speed)-smoothdx):
        smoothspeed[i] = np.average(speed[i-smoothdx:i+smoothdx])


    fig1 = plt.figure(1)
    ax1 = fig1.add_subplot(511)
    plt.plot(time_minutes,smoothspeed,'r',label='Speed')
    ax1.set_ylabel('Speed (KMH)')

    ax2 = fig1.add_subplot(512)
    plt.plot(time_minutes,elev,'g',label='Elevation')
    ax2.set_ylabel('Elev. (m)')

    ax3 = fig1.add_subplot(513)
    plt.plot(time_minutes,power,'b',label='Power')
    ax3.set_ylabel('Power (W)')

    ax4 = fig1.add_subplot(514)
    plt.plot(time_minutes, cad,'k',label='Cadence')
    ax4.set_ylabel('Cad. (RPM)')

    ax5 = fig1.add_subplot(515)
    plt.plot(time_minutes, hr,label='Cadence')
