...
def rankine_steamCycle(mdot,Plow,Phigh,Thigh,compressor_eta, turbine_eta):
    ...
    #state 3a - after high pressure for expanding isentropically until the dome and then accounting for turbine losses to that same pressure
    rFluid=set_gasSQ(rFluid,s3,1.0)
    P3a_isen = rFluid.P
    T3a_isen = rFluid.T
    h3a_isen = rFluid.enthalpy_mass
    h3a = h3 -turbine_eta*(h3-h3a_isen)
    rFluid.HP = h3a, P3a_isen
    states.append(rFluid.state,stage='Stage 3a')



    rFluid.TPQ = T3,P3a_isen,1
    states.append(rFluid.state,stage='Stage 3b')
    s3b = rFluid.entropy_mass
    h3b = rFluid.enthalpy_mass


   #Perfect turbine
    s3c_isentropic = s3b
    rFluid.SP = s3c_isentropic, Plow
    h4_isentropic = rFluid.enthalpy_mass
    deltah34_isentropic = h3b-h4_isentropic
    ...
    WdotIn = mdot*(h2-h1)
    QdotIn = mdot*((h3 - h2)+(h3b - h3a))
    WdotOut = mdot*((h3-h3a)+(h3b-h4))
    eta_thermal = (WdotOut-WdotIn)/QdotIn
    eta_carnot = 1-T1/T3
    eta_2nd    = eta_thermal/eta_carnot
    output_str = '{:35s}{:>4.2f} {}'
    print(output_str.format('Work Input:',WdotIn/1e3, 'kJ/kg'))
    print(output_str.format('Heat Input:',QdotIn/1e3, 'kJ/kg'))
    print(output_str.format('Work Output:',WdotOut/1e3, 'kJ/kg'))
...
def plotHSDiagram(rFluid, states):
    ...
    plt.plot(states.s,states.h,label='Rankine Cycle')
    plt.plot(states2.s,states2.h)
    for i, txt in enumerate(states.stage):
        plt.annotate(txt,(states.s[i],states.h[i]))
    plt.xlabel('Specific Entropy (J/kg-K)')
...
if __name__ == '__main__':
    ...
    rFluid, states, Q4, Tmin = rankine_steamCycle(mdot,Plow,Phigh,Thigh,compressor_efficiency,0.9)
